...
public class PaymentOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderPaymentOutboxMessage>> outboxMessage =
        ...
        if (outboxMessage.isPresent() && !outboxMessage.get().isEmpty()) {
            List<OrderPaymentOutboxMessage> outboxMessages = outboxMessage.get();
            log.info("Received {} OrderPaymentOutboxMessage with ids: {}, sending to message bus!",
                    outboxMessages.size(),
                    outboxMessages.stream().map(outboxMsg ->
                            outboxMsg.getId().toString()).collect(Collectors.joining(",")));
            outboxMessages.forEach(outboxMsg ->
                    paymentRequestMessagePublisher.publish(outboxMsg, this::updateOutboxStatus));
